...
probit_ep_glasso = function(X, Y, sampler, sampler_params, penalty = .01,
                            Sigma_init = NULL, beta_init = NULL, 
                            tol = 1e-5, max_iter = 200, n_mc_samples = 100,
                            corr = FALSE, pen_diag = FALSE,
                            verbose = FALSE) {
  ...
  while (dPrec > tol & iter <= max_iter) {
    ...
    for (i in 1:n_obs) {
      ...
      # E step
      moments = epmgpr::moments2(mu, Sigma, lb, ub, A)
      
      # M step
      E_sample_cov = E_sample_cov + moments$Sigma + tcrossprod(moments$mu - mu)
    }
