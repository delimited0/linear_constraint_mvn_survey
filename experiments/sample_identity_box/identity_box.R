...
Usage:
  identity_box.R (--method_conf=<method_conf>) (--dim_conf=<dim_conf>) (--half_width=<half_width>) (--sample_path=<sample_path>) (--timing_path=<timing_path>) [--seed=seed] [--n_threads=n_threads] 
...
progressr::with_progress({
  ...
  foreach(i = 1:n_settings, .inorder = FALSE, 
          .export = ls(globalenv())) %dorng% 
    {
      ...
      method_timing_path = 
        paste0(timing_path, "/width=", half_width, "/", method$method, "/")
      if (!dir.exists(method_sample_path)) 
        dir.create(method_sample_path, recursive=TRUE)
      if (!dir.exists(method_timing_path)) 
        dir.create(method_timing_path, recursive=TRUE)
      
      # save samples
      saveRDS(samples, paste0(method_sample_path, "d=", d))
      
      # save timings
      saveRDS(runtime, paste0(method_timing_path, "d=", d))
    } 
})
